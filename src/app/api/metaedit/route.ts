...
export async function POST(request: Request) {
  try {
    ...
    if (action === "heartbeat") return NextResponse.json({ ok: true, state: await readWorkspaceState(collaborator.id) });
    ...
    if (action === "create_annotation") {
      ...
      const result = await idempotent(key, async () => {
        ...
        const now = Date.now();
        await getD1().prepare(`INSERT INTO annotations (id, workspace_id, author_id, author_name, target_id, selector, component, source, text_snapshot, style_snapshot, comment, status, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'open', ?, ?)`).bind(id, WORKSPACE_ID, collaborator.id, collaborator.displayName, target.instanceId, target.selector, target.component, target.source, target.textSnapshot, JSON.stringify(target.styleSnapshot), comment, now, now).run();
        await recordActivity("annotation.created", collaborator, id, `${collaborator.displayName} annotated ${target.component} (#${target.instanceId}).`);
    ...
    if (action === "resolve_annotation") {
      ...
      await getD1().prepare(`UPDATE annotations SET status = 'resolved', updated_at = ? WHERE id = ?`).bind(Date.now(), annotationId).run();
    ...
    if (action === "propose_revision") {
      ...
      if (patch.some((operation) => operation.selector !== annotation.selector)) return NextResponse.json({ error: "This revision may only change the annotated target." }, { status: 400 });
      const result = await idempotent(key, async () => {
    ...
    if (action === "publish_revision") {
      ...
      await idempotent(key, async () => {
        ...
        await getD1().batch([
          ...
          getD1().prepare(`UPDATE annotations SET status = 'resolved', updated_at = ? WHERE id = ?`).bind(now, revision.annotationId),
        ]);
